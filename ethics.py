...
class Leader(agent.Agent):
    ...
    def findWeightedHappiness(self, agent, cell, placementByCell=None):
        w = HAPPINESSWEIGHTS
        return (w["foodSecurity"] * self.predictedFoodSecurityHappiness(agent, cell)
            + w["health"]       * agent.healthHappiness
            + w["wealth"]       * self.predictedWealthHappiness(agent, cell)
            + w["conflict"]     * self.predictedConflictHappiness(agent, cell)
    ...
    def countViableAgents(self, agents, placementKeys):
        count = 0
        for a in agents:
            key = placementKeys.get(a.ID)
            if key is None:
                continue
            x, y = key
            if self.ttlAfterMove(a, self.environment.grid[x][y]) >= 1.0:
                count += 1
        return count
    ...
    def bruteforcePlacements(self, agents, timestep):
        ...
        while stack:
            ...
            if idx == n:
                ...
                if score > bestScore or (score == bestScore and viable > bestViable):
                    bestScore = score
                    bestViable = viable
